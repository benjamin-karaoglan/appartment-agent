...
# Test import of 2023 data using server-side COPY

echo "=================================================="
echo "TEST DVF IMPORT - 2023 ONLY (Server-Side COPY)"
echo "=================================================="
echo ""
...
COPY dvf_staging FROM '/tmp/ValeursFoncieres-2023.txt' WITH (FORMAT csv, DELIMITER '|', HEADER true, NULL '', ENCODING 'UTF8');
...
SELECT 'Inserted records: ' || COUNT(*)::text FROM dvf_records WHERE data_year = 2023;
...
-- Verification
SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE property_type = 'Appartement') as appts,
       ...
       ROUND(AVG(price_per_sqm) FILTER (WHERE price_per_sqm > 0)::numeric, 2) as avg_price_sqm
FROM dvf_records;

SELECT sale_date, sale_price, address, postal_code, city, property_type, surface_area, rooms, ROUND(price_per_sqm::numeric, 2)
FROM dvf_records WHERE postal_code = '75006' ORDER BY sale_date DESC LIMIT 5;
EOF

echo "Starting import..."
START=$(date +%s)

docker exec -i "$DB_CONTAINER" psql -U "$DB_USER" -d "$DB_NAME" < /tmp/test_import.sql
